...
export async function createTicketPriority(req: Request, res: Response) {
  const { name, cssClass, enabled, slaPolicyId } = req.body;
  if (!name) {
    return res.status(400).json({ message: "name is required" });
  }
  try {
    const result = await db.query(
      `INSERT INTO ticket_priorities (name, enabled, css_class, sla_policy_id) VALUES ($1, $2, $3, $4) RETURNING *`,
      [name, enabled, cssClass, slaPolicyId]
    );
    res.status(201).json({
      data: result.rows[0],
...
export async function createBulkTicketPriorities(req: Request, res: Response) {
  ...
  try {
    ...
    for (const priority of priorities) {
      const { name, enabled, cssClass, slaPolicyId } = priority;
      if (!name) {
...
export async function updateTicketPriority(req: Request, res: Response) {
  ...
  try {
    const result = await db.query(
      `UPDATE ticket_priorities SET name = $1, enabled = $2, css_class = $3, sla_policy_id = $4 WHERE id = $5 RETURNING *`,
      [name, enabled, cssClass, slaPolicyId, id]
    );
